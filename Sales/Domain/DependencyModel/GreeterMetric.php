...
use Sales\Infrastructure\Persistence\Doctrine\Repository\DoctrineGreeterMetricRepository;
...
class GreeterMetric
{
    ...
    public function fetchSummaryResult(Connection $connection, string $salesId): array
    {
        ...
        match ($this->salesMetricType) {
            ...
            SalesMetricType::SUCCESSFULL_ASSIGNMENT => $this->applySuccessfullGreetingMetric($qb, $salesId),
        };
        $result = [
            'name' => $this->name,
            'target' => $this->target,
        ...
        if (!empty($this->dailyReminderTarget)) {
            ...
            match ($this->salesMetricType) {
                ...
                SalesMetricType::SUCCESSFULL_ASSIGNMENT => $this->applySuccessfullGreetingDailyAchievement($dailyAchievementQB, $salesId),
            };
    ...
    //
    protected function prepareSuccessfullGreetingMetric(QueryBuilder $qb, string $salesId): void
    {
        ...
        $qb->from('GreetingAssignment')
                ->innerJoin('GreetingAssignment', 'CustomerAssignment', 'CustomerAssignment',
                        "GreetingAssignment.CustomerAssignment_id = CustomerAssignment.id")
                ->andWhere($qb->expr()->eq('GreetingAssignment.status', "'$completedAssignmentStatus'"))
                ->andWhere($qb->expr()->eq('GreetingAssignment.Sales_id', ':salesId'))
                ->setParameter('salesId', $salesId);
        $this->evaluationType->applyToQuery($qb, 'GreetingAssignment.id');
    }
    protected function applySuccessfullGreetingMetric(QueryBuilder $qb, string $salesId): void
    {
        $this->prepareSuccessfullGreetingMetric($qb, $salesId);
        $this->recurrenceType->applyToQuery($qb, 'CustomerAssignment.completedTime', $this->recurrenceCount);
    }
    protected function applySuccessfullGreetingDailyAchievement(QueryBuilder $qb, string $salesId): void
    {
        $this->prepareSuccessfullGreetingMetric($qb, $salesId);
        $qb->setMaxResults(1);
